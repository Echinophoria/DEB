...
library(devtools)
install_github('https://github.com/Echinophoria/DEB_output')  # to make sure we load the last version
library(DEB.tools)
...
name <- file.path(path, 'parameters_Mytilus_edulis.csv')
p <- get_DEB_pars(name)  # DEB model parameters, and auxiliary
...
while (H > p$E_Hb  & H < p$E_Hj){
  ...
  if (p$kap * pC >= pS) {  # there is energy for somatic maintenance and maybe for growth, kappa rule fully applies
    ...
    dH = (pC - pS - pJ) 
    dV = 0
    if (dH < 0) {  # when there isn't energy enough to cover maintenance, the animal either dies or uses its tissue. In the case of mussels tissue is used, first the reproductive tissue, when exausted the structure is used. At this stage there is no reproductive tissue. So only energy from structure can be used to keep the animal alive.
...
while (H > p$E_Hp){
  ...
  if (p$kap * pC >= pS) {  # there is energy for somatic maintenance and maybe for growth, kappa rule fully applies
    ...
    dR = (pC - pS - pJ) 
    dV = 0
    if (dR < 0) {  # when there isn't energy enough to cover maintenance, the animal either dies or uses its tissue. In the case of mussels tissue is used, first the reproductive tissue, when exausted the structure is used. At this stage there is no reproductive tissue. So only energy from structure can be used to keep the animal alive.
      dV = dR/p$E_G   # Start of using tissue. Animal shrink! shell cannot shrink so is only the somatic tissue that is shrinking. That is why we follow V and L and not physical length.
